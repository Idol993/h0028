...
router.post('/', authenticate, (req: Request, res: Response): void => {
  try {
    ...
    ).get(member_id, currentWeek) as any

    if (suspension) {
      res.status(403).json({ success: false, error: '您本周已被停约，无法预约课程' })
      return
    }
    ...
    ).get(member_id) as any

    if (!pkg) {
      res.status(400).json({ success: false, error: '您没有可用的课时包' })
      return
    }
